...
summation_real_closure = Forall([P, S], Implies(Forall(x_multi, InSet(
    Px_etc, Real), domain=S), InSet(Sum(x_multi, Px_etc, domain=S), Real)))
summation_real_closure

summation_complex_closure = Forall([P, S], Implies(Forall(x_multi, InSet(
    Px_etc, Complex), domain=S), InSet(Sum(x_multi, Px_etc, domain=S), Complex)))
...
sum_zero_and_one = Forall(f, Equals(Sum(n, Operation(f, n), Interval(
    zero, one)), Add(Operation(f, zero), Operation(f, one))))
sum_zero_and_one

index_shift = Forall(f, Forall([a, b, c], Equals(Sum(x, Operation(f, x), Interval(a, b)), Sum(
    x, Operation(f, Sub(x, c)), Interval(Add(a, c), Add(b, c)))), domain=Integer))
index_shift
...
same_sums = Forall([f, g], Forall([m, n], Implies(Forall(k, Equals(Operation(f, k), Operation(g, k)), domain=Interval(
    m, n)), Equals(Sum(l, Operation(f, l), Interval(m, n)), Sum(l, Operation(g, l), Interval(m, n)))), domain=Integer))
same_sums
...
bounded_sum = Forall((S, f, g), Implies(Forall(x, LessThanEquals(
    fx, gx), domain=S), LessThanEquals(Sum(x, fx, domain=S), Sum(x, gx, domain=S))))
bounded_sum
