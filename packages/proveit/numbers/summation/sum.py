...
class Sum(OperationOverInstances):
    ...
    _operator_ = Literal(string_format='Sum', latex_format=r'\sum', theory=__file__)   
...
#
    ...
    def deduce_in_number_set(self, number_set, assumptions=USE_DEFAULTS):
        ...
        if number_set == Natural:
            ...
            raise ProofFailure(InSet(self, number_set), assumptions, "'deduce_in_number_set' not implemented for the %s set"%str(number_set))
        return thm.instantiate({P_op:P_op_sub, S:self.domain, Q_op:Q_op_sub}, relabel_map={x_multi:self.instance_vars}, 
                                assumptions=assumptions).derive_consequent(assumptions=assumptions)    
    ...
    def _formatted(self, format_type, **kwargs):
        ...
        if isinstance(self.domain,Interval):
            ...
            formatted_inner = self.operator.formatted(format_type)+r'_{'+self.index.formatted(format_type)+'='+lower+r'}'+r'^{'+upper+r'} '
            ...
            if has_explicit_conds:
                if has_explicit_ivars: 
                    formatted_inner += " | "
                formatted_inner += ', '.join(condition.formatted(format_type) for condition in explicit_conds) 
    ...
    def do_reduced_simplification(self, assumptions=USE_DEFAULTS):
        ...
        if isinstance(self.domain, Interval) and self.domain.lower_bound == self.domain.upper_bound:
            if len(self.instance_vars) == 1:
                return sum_single.instantiate({Operation(f, self.instance_vars):self.summand}).instantiate({a:self.domain.lower_bound})
...
#            self.r = extract_exp_base(self.summand)
        ...
        if not isinstance(self.domain,Interval):
            ...
            if self.domain.lower_bound == zero and self.domain.upper_bound == infinity:
                ...
                deduce_in_complex(x_val, assumptions)
                return fin_geom_sum.instantiate({x:x_val, m:m_val, k:k_val, l:l_val})
#        else:
#            print "Not a geometric sum!"
    ...
    def shift(self, shift_amount, assumptions=frozenset()):
        ...
        return index_shift.instantiate({f_op:f_op_sub, x:self.index}).instantiate({a:self.domain.lower_bound, b:self.domain.upper_bound, c:shift_amount})
    ...
    def join(self, second_summation, assumptions=frozenset()):
        ...
        if not isinstance(self.domain, Interval) or not isinstance(second_summation.domain, Interval):
        ...
        if self.domain.upper_bound == Sub(second_summation.domain.lower_bound, one):
        ...
        return sum_split.instantiate({Operation(f, self.instance_vars):self.summand}).instantiate({a:lower_bound, b:split_index, c:upper_bound, x:self.indices[0]}).derive_reversed()
    ...
    def split(self, split_index, side='after', assumptions=frozenset()):
        ...
        if side=='before' and self.domain.upper_bound==split_index: return self.split_off_last()
        if side=='after' and self.domain.lower_bound==split_index: return self.split_off_first()
        if isinstance(self.domain, Interval) and len(self.instance_vars) == 1:
            ...
            deduce_in_integer(self.domain.upper_bound, assumptions)
            deduce_in_integer(split_index, assumptions)
            # Also needs lower_bound <= split_index and split_index < upper_bound
            return sum_split.instantiate({Operation(f, self.instance_vars):self.summand}).instantiate({a:self.domain.lower_bound, b:split_index, c:self.domain.upper_bound, x:self.indices[0]})
        raise Exception("split_off_last only implemented for a summation over a Interval of one instance variable")
    ...
    def split_off_last(self, assumptions=frozenset()):
        ...
        if isinstance(self.domain, Interval) and len(self.instance_vars) == 1:
            ...
            return sum_split_last.instantiate({Operation(f, self.instance_vars):self.summand}).instantiate({a:self.domain.lower_bound, b:self.domain.upper_bound, x:self.indices[0]})
        raise Exception("split_off_last only implemented for a summation over a Interval of one instance variable")
    ...
    def split_off_first(self, assumptions=frozenset()):
        ...
        if isinstance(self.domain, Interval) and len(self.instance_vars) == 1:
            ...
            deduce_in_integer(self.domain.upper_bound, assumptions)
            # Also needs lower_bound < upper_bound
            return sum_split_first.instantiate({Operation(f, self.instance_vars):self.summand}).instantiate({a:self.domain.lower_bound, b:self.domain.upper_bound, x:self.indices[0]})
        raise Exception("split_off_last only implemented for a summation over a Interval of one instance variable")
    ...
    def factor(self, the_factor, pull="left", group_factor=False, group_remainder=None, assumptions=frozenset()):
        ...
        summand_factor_eq = self.summand.factor(the_factor, pull, group_factor=False, group_remainder=True, assumptions=summand_assumptions)
        summand_instance_equivalence = summand_factor_eq.generalize(self.indices, domain=self.domain).checked(assumptions)
        eq = Equation(self.instance_substitution(summand_instance_equivalence).checked(assumptions))
        factor_operands = the_factor.operands if isinstance(the_factor, Mult) else the_factor
        x_dummy, z_dummy = self.safe_dummy_vars(2)
        # Now do the actual factoring by reversing distribution
        if pull == 'left':
            Pop, Pop_sub = Operation(P, self.indices), summand_factor_eq.rhs.operands[-1]
            x_sub = factor_operands
            z_sub = []
        elif pull == 'right':
            Pop, Pop_sub = Operation(P, self.indices), summand_factor_eq.rhs.operands[0]
            x_sub = []
            z_sub = factor_operands
        # We need to deduce that the_factor is in Complex and that all instances of Pop_sup are in Complex.
        deduce_in_complex(factor_operands, assumptions=assumptions)
        deduce_in_complex(Pop_sub, assumptions=assumptions | {InSet(idx, self.domain) for idx in self.indices}).generalize(self.indices, domain=self.domain).checked(assumptions)
        # Now we instantiate distribut_through_summation_rev
        spec1 = distribute_through_summation_rev.instantiate({Pop:Pop_sub, S:self.domain, y_etc:self.indices, x_etc:Etcetera(Multi_variable(x_dummy)), z_etc:Etcetera(Multi_variable(z_dummy))}).checked()
        eq.update(spec1.derive_conclusion().instantiate({Etcetera(Multi_variable(x_dummy)):x_sub, Etcetera(Multi_variable(z_dummy)):z_sub}))
        if group_factor and len(factor_operands) > 1:
            eq.update(eq.eq_expr.rhs.group(end_idx=len(factor_operands), assumptions=assumptions))
