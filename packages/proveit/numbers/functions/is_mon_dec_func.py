...
def deduce_as_mon_dec_func(fxn, *, domain=None,
    ...
    if membership is not None:
        required_expr = IsMonDecFunc(fxn, domain)
        if not isinstance(membership, Judgment):
            raise ValueError(
                "'deduce_as_mon_dec_func' should have returned "
                "a proven Judgment")
        if membership.expr != required_expr:
            raise ValueError(
                "'deduce_as_mon_dec_func' should have proven and "
                "returned %s but returned %s instead"%required_expr)
        return membership
