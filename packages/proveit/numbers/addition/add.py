...
class Add(Operation):
    ...
    def __init__(self, *operands):
        ...
        subtraction_positions = [_k for _k, operand in enumerate(operands) if Add._isNegatedOperand(operand)]
        styles = {'subtraction_positions': '(' + ' '.join(str(pos) for pos in subtraction_positions) +')'}
    ...
    def style_options(self):
        ...
        options.add('subtraction_positions', "Position(s) to use subtraction notation instead of adding the negation at the specified indices")
        return options

    def with_subtraction_at(self, *subtraction_positions):
        return self.with_styles(subtraction_positions='(' + ' '.join(str(pos) for pos in subtraction_positions) + ')')

    def subtraction_positions(self):
        '''
        Return a list of subtraction notation positions according to the current style setting.
        '''
        return [int(pos_str) for pos_str in self.get_style('subtraction_positions').strip('()').split(' ') if pos_str != '']
    ...
    def _formatted(self, format_type, **kwargs):
        ...
        if len(subtraction_positions) > 0 and len(self.operands)>1:
            ...
            for pos in subtraction_positions:
                ...
                if pos==0: implicit_first_operator=False
                if isinstance(operand, Neg):
                    ...
                    if isinstance(operand.lambda_map.body, Neg):
                        ...
                        operands[pos] = ExprRange(operand.lambda_map.parameter, operand.lambda_map.body.operand, operand.start_index, operand.end_index) \
                            .with_styles(**operand.get_styles())
                elif pos==0: implicit_first_operator=False # not negated after all -- revert to the "implicit first operator" default
            return Operation._formattedOperation(format_type, operators, operands, self.wrap_positions(), self.get_style('justification'), implicit_first_operator=implicit_first_operator, **kwargs)
        else:
            return Operation._formattedOperation(format_type, self.operator, self.operands, self.wrap_positions(), self.get_style('justification'), **kwargs)

    def remake_constructor(self):
        # Added by JML on 9/10/19
        if len(self.operands)==2 and self.subtraction_positions() == (1,) and Add._isNegatedOperand(self.operands[1]):
    ...
    def remake_with_style_calls(self):
        ...
        if subtraction_positions != default_subtraction_positions:
            call_strs.append('with_subtraction_at(' + ','.join(str(pos) for pos in subtraction_positions) + ')')
        return call_strs
    ...
    def deduce_negation(self, rhs, assumptions=USE_DEFAULTS):
        ...
        if len(self.terms) != 2:
            raise Exception("deduce_negation implemented only when there are two and only two added terms")
        deduction = negated_add.instantiate({a:self.terms[0], b:self.terms[1], c:rhs}, assumptions=assumptions)
        return deduction
    ...
    def deduce_subtraction(self, rhs, assumptions=USE_DEFAULTS):
        ...
        if len(self.terms) != 2:
            raise Exception("deduce_subtraction implemented only when there are two and only two added terms")
        deduction = subtract_from_add.instantiate({a:self.terms[0], b:self.terms[1], c:rhs}, assumptions=assumptions)
        return deduction
    ...
    def deduce_reversed_subtraction(self, rhs, assumptions=USE_DEFAULTS):
        ...
        if len(self.terms) != 2:
            raise Exception("subtract_from_add_reversed implemented only when there are two and only two added terms")
        deduction = subtract_from_add_reversed.instantiate({a:self.terms[0], b:self.terms[1], c:rhs}, assumptions=assumptions)
        return deduction
    ...
    def conversion_to_multiplication(self, assumptions=USE_DEFAULTS):
        ...
        if not all(operand==self.operands[0] for operand in self.operands):
            raise ValueError("'as_mult' is only applicable on an 'Add' expression if all operands are the same: %s"%str(self))
    ...
    def cancelation(self, idx1, idx2, assumptions=USE_DEFAULTS):
        ...
        if Neg(self.operands[idx1]) == self.operands[idx2]:
            ...
            triple_thms = (add_cancel_triple_12, add_cancel_triple_13, add_cancel_triple_23)
            ...
            triple_thms = (add_cancel_triple_21, add_cancel_triple_31, add_cancel_triple_32)
        ...
        if len(self.operands)==2:
            ...
            spec.inner_expr().rhs.with_subtraction_at(*[update_pos(p) for p in sub_positions])
            return spec
    ...
    def zero_elimination(self, idx, assumptions=USE_DEFAULTS):
        ...
        if len(self.operands)==2:
            if idx==0:
                return elim_zero_left.instantiate({a:self.operands[1]}, assumptions=assumptions)
            else:
                return elim_zero_right.instantiate({a:self.operands[0]}, assumptions=assumptions)
        _a = self.operands[:idx]
        _b = self.operands[idx+1:]
        _i = num(len(_a))
        _j = num(len(_b))
        return elim_zero_any.instantiate({i:_i, j:_j, a:_a, b:_b}, assumptions=assumptions)
    ...
    def deduce_zero_from_neg_self(self, assumptions=USE_DEFAULTS):
        ...
        from proveit.numbers import Neg
        if len(self.operands) != 2:
            raise IndexError("Expecting two operands.  Use substitution and inner_expr() for more than two operands")
        ...
        return add_neg_self.instantiate({x:self.terms[0]}, assumptions=assumptions)
    """
    ...
    def _createDict(self, assumptions=USE_DEFAULTS):
        ...
        for _i, val in enumerate(self.operands):
            ...
            if isinstance(val, Neg):
                ...
                val = val.operands[-1]
            if isinstance(val, Numeral) or (is_irreducible_value(val) and not isinstance(val, Literal)):
    ...
    def do_reduced_simplification(self, assumptions=USE_DEFAULTS, **kwargs):
        ...
        if len(order)==1:
            ...
            if (all(operand==expr.operands[0] for operand in expr.operands)
                ...
                eq.update(sub.substitution(expr.inner_expr().operands[0], assumptions))
                return eq.relation

        # simplify the combined terms
        for _i, operand in enumerate(expr.operands):
            if isinstance(operand, Add):
                expr = eq.update(expr.inner_expr().operands[_i].simplification(assumptions))
            elif isinstance(operand, Mult):
                if isinstance(operand.operands[0], Add):
                    expr = eq.update(expr.inner_expr().operands[_i].operands[0].simplification(assumptions))
                if isinstance(expr.operands[_i].operands[0], Add) and len(expr.operands[_i].operands[0].operands) == 1:
                    ...
                    sub = single_add.instantiate({x:expr.operands[_i].operands[0].operands[0]})
                    # print("single Add", sub)
                    expr = eq.update(sub.substitution(expr.inner_expr().operands[_i].operands[0], assumptions))
    ...
    def _integerBinaryEval(self, assumptions=USE_DEFAULTS):
        ...
        if len(abs_terms)!=2 or not all(is_literal_int(abs_term) for abs_term in abs_terms):
    ...
    def do_reduced_evaluation(self, assumptions=USE_DEFAULTS, **kwargs):
        ...
        abs_terms = [term.operand if isinstance(term, Neg) else term for term in self.terms]
        if len(abs_terms)==2 and all(is_literal_int(abs_term) for abs_term in abs_terms):
    ...
    def subtraction_folding(self, term_idx=None, assumptions=frozenset()):
        ...
        if len(expr.terms) > 2:
            ...
            expr = expr.group(0, -1)
        return add_neg_as_subtract.instantiate({x:expr.operands[0], y:expr.operands[-1].operand})
    ...
    def deduce_in_number_set(self, number_set,assumptions=USE_DEFAULTS):
        ...
        from proveit.numbers.addition._theorems_ import (
                add_int_closure_bin,add_int_closure, add_nat_closure_bin, add_nat_closure,
                add_nat_pos_closure, add_nat_pos_from_non_neg,
                add_real_closure_bin, add_real_closure,
                add_real_non_neg_closure, add_real_non_neg_closure_bin,
                add_real_pos_closure, add_real_pos_closure_bin, add_real_pos_from_non_neg,
                add_complex_closure, add_complex_closure_bin)
        ...
        if number_set == Integer:
            if len(self.operands) == 2:
                return add_int_closure_bin.instantiate({a: self.operands[0], b: self.operands[1]}, assumptions=assumptions)
            return add_int_closure.instantiate({i: num(len(self.operands)), a: self.operands}, assumptions=assumptions)
        ...
        if number_set == RealPos:
            if len(self.operands) == 2:
                return add_real_pos_closure_bin.instantiate({a: self.operands[0], b: self.operands[1]}, assumptions=assumptions)
            return add_real_pos_closure.instantiate({i: num(len(self.operands)), a: self.operands}, assumptions=assumptions)
        if number_set == RealNonNeg:
            if len(self.operands) == 2:
                return add_real_non_neg_closure_bin.instantiate({a: self.operands[0], b: self.operands[1]}, assumptions=assumptions)
            return add_real_non_neg_closure.instantiate({i: num(len(self.operands)), a: self.operands}, assumptions=assumptions)
        if number_set == Real:
            if len(self.operands) == 2:
                return add_real_closure_bin.instantiate({a: self.operands[0], b: self.operands[1]}, assumptions=assumptions)
            return add_real_closure.instantiate({i: num(len(self.operands)), a: self.operands}, assumptions=assumptions)
        if number_set == Complex:
            if len(self.operands) == 2:
                return add_complex_closure_bin.instantiate({a:self.operands[0], b: self.operands[1]}, assumptions=assumptions)
            return add_complex_closure.instantiate({i:num(len(self.operands)), a: self.operands}, assumptions=assumptions)
        msg = "'deduce_in_number_set' not implemented for the %s set"%str(number_set)
    ...
    def deduce_difference_in_natural_pos(self, assumptions=USE_DEFAULTS):
        ...
        if len(self.terms) != 2:
            raise ValueError("deduce_difference_in_natural_pos only applicable "
                               "when there are two terms, got %s"%self)
        if not isinstance(self.terms[1], Neg):
            raise ValueError("deduce_difference_in_natural_pos only applicable "
                               "for a subtraction, got %s"%self)
        ...
        return thm.instantiate({a:self.terms[0], b:self.terms[1].operand},
                               assumptions=assumptions)
    def deduce_strict_increase(self, lower_bound_term_index, assumptions=frozenset()):
        '''
    ...
    def deduce_strict_decrease(self, upper_bound_term_index, assumptions=frozenset()):
        '''
    ...
    def factorization(self, the_factor, pull="left", group_factor=True, assumptions=USE_DEFAULTS):
        '''
        ...
        for _i in range(len(expr.terms)):
            ...
            if hasattr(term, 'factorization'):
                term_factorization = term.factorization(the_factor, pull, group_factor=group_factor, group_remainder=True, assumptions=assumptions)
                ...
                if term != the_factor:
                    raise ValueError("Factor, %s, is not present in the term at index %d of %s!"%(the_factor, _i, self))
                factored_term = Mult(one, term) if pull=='right' else Mult(term, one)
                term_factorization = factored_term.simplification(assumptions).derive_reversed(assumptions)
                _b.append(one)
    ...
    def commutation(self, init_idx=None, final_idx=None, assumptions=USE_DEFAULTS):
        '''
        ...
        eq = apply_commutation_thm(self, init_idx, final_idx, commutation, leftward_commutation, rightward_commutation, assumptions)
        '''
    ...
    def group_commutation(self, init_idx, final_idx, length, disassociate=True, assumptions=USE_DEFAULTS):
        '''
        ...
        '''
        return group_commutation(self, init_idx, final_idx, length, disassociate, assumptions)
    ...
    def association(self, start_idx, length, assumptions=USE_DEFAULTS):
        ...
        eq = apply_association_thm(self, start_idx, length, association, assumptions)
...
InnerExpr.register_equivalence_method(Add, 'group_commutation', 'group_commuted', 'group_commute')
...
InnerExpr.register_equivalence_method(Add, 'cancelations', 'all_canceled', 'all_cancel')
InnerExpr.register_equivalence_method(Add, 'zero_elimination', 'eliminated_zero', 'eliminate_zero')
InnerExpr.register_equivalence_method(Add, 'zero_eliminations', 'eliminated_zeros', 'eliminate_zeros')
