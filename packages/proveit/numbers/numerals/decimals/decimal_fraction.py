...
class Decimal_fraction(Operation):
    ...
    def __init__(self, integer_part, fractional_part):
        Operation.__init__(self, Decimal_fraction._operator_, [integer_part, fractional_part])
        ...
        if not all(isinstance(part, WholeDecimal) for part in (integer_part, fractional_part)):
            raise Exception('A Decimal_fraction may only be composed of WholeNumber integer and fractional parts')
        
    def formatted(self, format_type, fence=False):
        return self.integer_part.formatted(format_type, False) + '.' + self.fractional_part.formatted(format_type, False)
