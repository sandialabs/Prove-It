...
class Relation:
    ...
    def __init__(self, rel_expr):
        ...
        if not isinstance(
                rel_expr,
                OrderingRelation) and not isinstance(
                rel_expr,
                Equals):
            raise ValueError(
                'Relation must be initialized with an OrderingRelation or Equals expression')
    ...
    def update(self, next_rel_expr):
        ...
        if not isinstance(
                ...
                Equals):
            raise ValueError(
                'Relation may updated only with an OrderingRelation or Equals expression')
        if isinstance(
                self.rel_expr,
                OrderingRelation) or (
                isinstance(
                self.rel_expr,
                Equals) and isinstance(
                    next_rel_expr,
                Equals)):
            self.rel_expr = self.rel_expr.apply_transitivity(
                next_rel_expr).checked({self.rel_expr, next_rel_expr})
        else:  # Updating an equals with an inequality!
            self.rel_expr = next_rel_expr.apply_transitivity(self.rel_expr).checked(
                {self.rel_expr, next_rel_expr})  # .derive_reversed()
