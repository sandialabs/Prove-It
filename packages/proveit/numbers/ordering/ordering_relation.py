...
class OrderingRelation(TransitiveRelation):
    ...
    def derive_shifted(
        ...
        raise NotImplementedError(
            'derive_shifted is implemented for each specific OrderingRelation')
    ...
    def derive_added(self, other_ordering_relation, assumptions=frozenset()):
        ...
        if not (isinstance(other, OrderingRelation)):
            raise ValueError(
                "Expecting 'other_ordering_relation' to be an OrderingRelation")
        ...
        return shifted_self.apply_transitivity(
