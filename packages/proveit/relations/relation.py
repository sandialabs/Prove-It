...
class Relation(ClassMembership):
    r'''
    Base class for generic relations.  Examples are Equals, NotEquals, Less,
    Subset, etc.  All Relations are class memberships indicating that the
    left-hand side (lhs) belongs to the class of objects that satisfy the
    particular relationship with the right-hand side (rhs).  For example,
    given x < 5, x is in the class of objects that are less than 5. 
    '''
    ...
    def __init__(self, operator, normal_lhs, normal_rhs, *, styles):
        ...
        ClassMembership.__init__(self, operator, normal_lhs, normal_rhs,
                                 styles=styles)
    ...
    def formatted(self, format_type, **kwargs):
        '''
        Return the Relation formatted a 'string' or 'latex'.
        '''
        # Don't use ClassMembership.formatted for relations.
        return Operation.formatted(self, format_type, **kwargs)

    def string(self, **kwargs):
        '''
        Return a string representation of the Relation.
        '''
        # Don't use ClassMembership.string for relations.
        return Operation.string(self, **kwargs)

    def latex(self, **kwargs):
        '''
        Return a latex-formatted representation of the Relation.
        '''
        # Don't use ClassMembership.latex for relations.
    ...
    def conclude(self, **defaults_config):
        ...
        return ClassMembership.conclude(self)
