...
class IsVecSpace(ClassMembership):
    '''
    IsVecSpace denotes membership in the class of vector spaces
    ...
    _operator_ = Literal(
            string_format=r'IsVecSpace', latex_format=r'\textrm{IsVecSpace}',
            theory=__file__)
    ...
    def __init__(self, space, field, *, styles=None):
        ClassMembership.__init__(self, IsVecSpace._operator_, 
    ...
    def get_field(field=None, *, may_be_none=False):
        ...
        if field is not None:
            return field
        if IsVecSpace.default_field is not None:
            return IsVecSpace.default_field
        if not may_be_none:
    ...
    def yield_known_vec_spaces(vec, *, field=None):
        ...
        for membership in InSet.yield_known_memberships(vec):
            # Check if the membership domain is a vector space over the
            # specified field.
            domain = membership.domain
            vec_space = None
            if field is None and IsVecSpace.has_known_membership(domain):
                vec_space = domain
            elif (field is not None and
                      IsVecSpace(domain, field).proven()):
                vec_space = domain
            else:
                try:
    ...
    def known_vec_space(vec, *, field=None):
        ...
        field = IsVecSpace.get_field(field, may_be_none=True)
        try:
            return next(IsVecSpace.yield_known_vec_spaces(vec, field=field))
        ...
        for vec in vecs:
            if isinstance(vec, ExprRange):
                ...
                with defaults.temporary() as tmp_defaults:
                    ...
                    body = IsVecSpace.known_vec_space(vec.body, field=field)
                vec_spaces.append(
                    ExprRange(vec.parameter, body,
                              vec.true_start_index, vec.true_end_index))
            else:
                vec_spaces.append(IsVecSpace.known_vec_space(vec, field=field))
    ...
    def common_known_vec_space(vecs, *, field=None):
        ...
        for vec in vecs:
            cur_vec_spaces = set(IsVecSpace.yield_known_vec_spaces(
                    vec, field=field))
    ...
    def known_field(vec_space):
        ...
        '''
        try:
            return next(IsVecSpace.yield_known_fields(vec_space))
        except StopIteration:
...
def deduce_as_vec_space(expr, *, field=None, **defaults_config):
    ...
    membership = None
    if field is not None and IsVecSpace(expr, field).proven():
        # Already known as an appropriate vector space.
        return IsVecSpace(expr, field).prove()
...
def including_vec_space(subset, *, field):
    ...
    if field is not None:
        if IsVecSpace.known_field(vec_space) != field:
            raise ValueError(
