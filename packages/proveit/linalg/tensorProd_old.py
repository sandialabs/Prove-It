...
from proveit.basiclogic.generic_ops import AssociativeOperation
from proveit.common import f, x, y, a_etc, x_etc, y_etc, z_etc, alpha, S
...
class TensorProd(AssociativeOperation):
    ...
    def equate_factors(self, tensor_equality):
        ...
        if not isinstance(
                tensor_equality.lhs,
                TensorProd) or not isinstance(
                tensor_equality.rhs,
                TensorProd):
            raise ValueError(
                "tensor_equality should be an Equals expression of tensor products")
        if len(
                tensor_equality.lhs.factors) != len(
                tensor_equality.rhs.factors):
            raise ValueError(
                "tensor_equality should be an Equals expression of tensor products with the same number of factors")
        ...
        if not self == tensor_equality.lhs:
            raise ValueError(
                "tensor_equality should be an Equals expression of tensor products with 'self' on one side of the equality")
        for k, (factor1, factor2) in enumerate(
                zip(tensor_equality.lhs.factors, tensor_equality.rhs.factors)):
            if factor1 != factor2:
                if tensor_equality.lhs.factors[:k] != tensor_equality.rhs.factors[
                        :k] or tensor_equality.lhs.factors[k + 1:] != tensor_equality.rhs.factors[k + 1:]:
                    raise ValueError(
                        "tensor_equality should be an Equals expression of tensor products that are the same except for only one factor")
                return tensor_prod_equiv_by_elimination.instantiate(
                    {a_etc: tensor_equality.lhs.factors[:k], x: factor1, y: factor2, z_etc: tensor_equality.lhs.factors[k + 1:]})
...
TENSOR_PROD = Literal(pkg,
                      ...
                      {STRING: r'otimes',
                       LATEX: r'\otimes'},
                      operation_maker=lambda operands: TensorProd(*operands))
...
class TensorExp(Operation):
    ...
    def do_reduced_simplification(self, assumptions=frozenset()):
        ...
        if self.exponent == one:
            ...
            raise ValueError(
                'Only trivial simplification is implemented (tensor exponent of one)')


TENSOR_EXP = Literal(pkg,
                     'TENSOR_EXP',
                     {STRING: r'^otimes',
                      LATEX: r'^{\otimes}'},
                     operation_maker=lambda operands: TensorExp(*operands))
