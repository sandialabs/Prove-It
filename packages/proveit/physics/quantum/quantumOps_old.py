...
class Bra(Operation):
    ...
    def formatted(self, format_type, fence=False):
        if format_type == LATEX:
            return r'\langle ' + \
                self.label.formatted(format_type, fence=False) + r' \rvert'
        else:
...
class Ket(Operation):
    ...
    def formatted(self, format_type, fence=False, no_lvert=False):
        ...
        if format_type == LATEX:
            return left_str + \
                self.label.formatted(format_type, fence=False) + r' \rangle'
        else:
            return left_str + \
...
class RegisterBra(Operation):
    ...
    def formatted(self, format_type, fence=False):
        ...
        if format_type == LATEX:
            return r'\prescript{}{' + formatted_size + \
                r'}\langle ' + formatted_label + r' \rvert'
        else:
            return '{' + formatted_size + '}_<' + formatted_label + '|'


REGISTER_BRA = Literal(
    pkg,
    'REGISTER_BRA',
    operation_maker=lambda operands: RegisterBra(
        *operands))
...
class RegisterKet(Operation):
    ...
    def formatted(self, format_type, fence=False, no_lvert=False):
        ...
        if no_lvert:
            left_str = ''
        if format_type == LATEX:
            return left_str + formatted_label + \
                r' \rangle_{' + formatted_size + '}'
        else:
            return left_str + formatted_label + '>_{' + formatted_size + '}'


REGISTER_KET = Literal(
    pkg,
    'REGISTER_KET',
    operation_maker=lambda operands: RegisterKet(
        *operands))
...
MEAS = Literal(pkg, 'MEAS', {LATEX: r'{\cal M}'},
               operation_maker=lambda operands: Meas(*operands))
