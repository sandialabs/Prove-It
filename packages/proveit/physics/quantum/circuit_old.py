...
INPUT = Literal(pkg, 'INPUT', operation_maker=lambda operands: Input(
    *operands))  # An input state (entering the left of the circuit)
...
OUTPUT = Literal(pkg, 'OUTPUT', operation_maker=lambda operands: Output(
    *operands))  # An output state (exiting the right of the circuit)
...
class Target(Operation):
    ...
    def formatted(self, format_type, fence=False):
        formatted_gate_operation = self.target_gate.formatted(
...
TARGET = Literal(
    pkg, 'TARGET', {
        STRING: 'TARGET', LATEX: r'\targ'}, lambda operands: Target(
            *operands))
...
MULTI_WIRE = Literal(
    pkg,
    'MULTI_WIRE',
    operation_maker=lambda operands: MultiWire(
        *operands))
...
class Circuit(Operation):
                ...
                for col in range(tensor.shape[1]):
                    if (row, col) in tensor:
                        ...
                        if isinstance(cell, ExpressionTensor):
                            ...
                            max_depth_along_row = max(
                                max_depth_along_row, sub_depth + 1)
    ...
    # return Circuit(ExpressionTensor.substituted(self, expr_map,
    # operation_map=operation_map, relabel_map=relabel_map,
    ...
    def _generate_nested_row_indices(self, circuit_tensor):
        ...
        Each nested row index is a list with elements corresponding to each nesting level.
        '''
        for cur_level_row, deepest_tensor_along_row in enumerate(
                self.deepest_nested_tensor_along_row[circuit_tensor]):
            if deepest_tensor_along_row is None:
                yield [cur_level_row]
            else:
                for sub_nested_row in self._generate_nested_row_indices(
                        deepest_tensor_along_row):
    ...
    def generate_circuit_elements_along_row(self, nested_row_idx):
        ...
        '''
        for circuit_elem in Circuit._GenerateCircuitElementsAlongRow(
                self.tensor, nested_row_idx, 0):
            yield circuit_elem

    @staticmethod
    def _GenerateCircuitElementsAlongRow(
            circuit_tensor, nested_row_idx, level):
        '''
        Generate the circuit elements, as (level, circuit, row, column) tuples, along a particular
        ...
        for column in range(circuit_tensor.shape[1]):
            if (row, column) in circuit_tensor:
                ...
                if isinstance(cell, ExpressionTensor):
                    # nested Tensor
                    for circuit_elem in Circuit._GenerateCircuitElementsAlongRow(
                            cell, nested_row_idx, level + 1):
                        yield circuit_elem
                if isinstance(
                        cell,
                        Output) or cell == WIRE_UP or cell == WIRE_DN:
                    yield level, circuit_tensor, row, column
                    # nothing after the output or when the wire goes up/down
                    # (won't work if starting a new wire -- needs work)
                    break
            yield level, circuit_tensor, row, column
    ...
    def number_of_nested_rows(self, circuit_tensor, row):
        ...
        if deepest_tensor_along_row is None:
            return 1
        return sum(
            self.number_of_nested_rows(
                deepest_tensor_along_row,
                sub_row) for sub_row in range(
                deepest_tensor_along_row.shape[0]))
    ...
    def formatter(self, format_type, fence=False):
        ...
        if format_type == LATEX:
            ...
            for nested_row_idx in self.generate_nested_row_indices():
                ...
                if sum(nested_row_idx) > 0:
                    yield r' \\ '  # previous row has ended
                for level, circuit_tensor, row, column in self.generate_circuit_elements_along_row(
                        nested_row_idx):
                    if not (row, column) in circuit_tensor:
                        ...
                        if level < len(nested_row_idx) - 1:
                            ...
                            if sum(nested_row_idx[level:]) == 0:
                                ...
                                num_multi_gate_rows = self.number_of_nested_rows(
                                    circuit_tensor, row)
                            ...
                            # junction, but the instruction here just needs to
                            # continue the horizontal wire
                            yield r' & \qw'
                        elif elem == CTRL_UP:
                            ...
                            yield r' & \qwx[' + str(Circuit._NearestTarget(circuit_tensor, row, column, -1)) + r'] \qw'
                        elif elem == WIRE_DN:
                            yield r' & \qwx[' + str(Circuit._NearestTarget(circuit_tensor, row, column, 1)) + r'] \qw'
                        elif elem == CTRL_UPDN:
                            yield r' & \ctrl{' + str(Circuit._NearestTarget(circuit_tensor, row, column, -1)) + '}'
...
CIRCUIT = Literal(
    pkg,
    'CIRCUIT',
    operation_maker=lambda operands: Circuit(
        operands[0]))
