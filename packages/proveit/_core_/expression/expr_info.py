...
class ExpressionInfo:
    ...
    def _getEnumeratedExpressions(self):
        ...
        return ordered_dependency_nodes(
            self.expr, lambda expr: expr._sub_expressions)
    ...
    def __repr__(self):
        ...
        expr_num_map = {
            expr: k for k,
            expr in enumerate(enumerated_expressions)}
        out_str = ''
        for k, expr in enumerate(enumerated_expressions):
            ...
            if self.show_details:
                ...
                if len(expr._core_info) > 4:
                    out_str += indent + 'extra_core_info: ' + \
            ...
            if isinstance(expr, NamedExprs):
                ...
                if hasattr(expr, 'operand'):  # has a single operand
                    ...
                    out_str += indent + r'operands: ' + \
                        str(expr_num_map[expr.operands]) + '\n'
            elif isinstance(expr, Conditional):
                out_str += indent + r'value: ' + \
                    str(expr_num_map[expr.value]) + '\n'
                out_str += indent + r'condition: ' + \
                    str(expr_num_map[expr.condition]) + '\n'
                ...
                out_str += indent + r'body: ' + \
                    str(expr_num_map[expr.body]) + '\n'
    ...
    def _repr_html_(self):
        ...
        # generate the html as a table with the enumerated expressions on the
        # rows.
        ...
        for k, expr in enumerate(enumerated_expressions):
            ...
            if isinstance(expr, NamedExprs):
                for key in list(expr.keys()):
                    sub_expressions += '%s: %d<br>' % (key,
                                                       expr_num_map[expr[key]])
            elif isinstance(expr, IndexedVar):
                sub_expressions = 'variable:&nbsp;%d<br>' % (
                    expr_num_map[expr.var])
                if hasattr(expr, 'index'):
                    sub_expressions += 'index:&nbsp;%d<br>' % (
                ...
                if hasattr(expr, 'operand'):  # has a single operand
                    ...
                    sub_expressions += 'operands:&nbsp;%d<br>' % (
                        expr_num_map[expr.operands])
            elif isinstance(expr, Conditional):
                ...
                if hasattr(
                        expr,
                        'parameter') and not isinstance(
                        expr.parameter,
                        ExprRange):  # has a single parameter
                    sub_expressions = 'parameter:&nbsp;%s<br>' % (
                        expr_num_map[expr.parameter])
                else:
                    sub_expressions = 'parameters:&nbsp;%s<br>' % (
                        expr_num_map[expr.parameters])
                sub_expressions += 'body:&nbsp;%d<br>' % (
                    expr_num_map[expr.body])
            elif isinstance(expr, IndexedVar):
                sub_expressions += 'var:&nbsp;%d<br>' % (
                    expr_num_map[expr.var])
                if hasattr(expr, 'index'):
                    sub_expressions += 'index:&nbsp;%d<br>' % (
                        expr_num_map[expr.index])
            ...
            if self.show_details and expr.__class__ not in \
                ...
                # not a core expression so show the actual class when showing
                # the details
