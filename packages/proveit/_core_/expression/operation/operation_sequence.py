    ...
    def __init__(self, operators, operands, styles=None):
        ...
        for k, operator in enumerate(operators):
            if isinstance(operator, ExprRange):
                operand_idx = k if isinstance(operands[k], ExprRange) else k+1
                operand_range_indices.add(operand_idx)
                operand = operands[operand_idx]
                if not isinstance(operand, ExprRange):
                    raise ValueError("In an operation sequence, an range of operations must correspond with a range of operands (with operands preceeding operations or vice-versa)")
                if operand.start_index != operator.start_index or operand.end_index != operator.end_index:
                    raise ValueError("In an operation sequence, operation and operand ranges in correspondence must have the same start and end indices")
        for k, operand in enumerate(operands):
            if isinstance(operand, ExprRange) and k not in operand_range_indices:
