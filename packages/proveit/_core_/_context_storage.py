    ...
    def setSpecialExpressions(self, names, definitions, kind):
        ...
        if kind=='common':
            ...
            if kind=='axiom':
                ...
                definitions = {name:Axiom(expr, self.context, name)
                              for name, expr in definitions.items()}
            elif kind == 'theorem':
                self._theorem_names = None # force a reload
                # Convert definitions from expressions to Theorem 
                # Proofs.
                definitions = {name:Theorem(expr, self.context, name) 
                               for name, expr in definitions.items()}
    ...
    def _getSpecialObject(self, kind, name):
        ...
        try:
            ...
            if (ContextFolderStorage.owns_active_storage and 
                    context_folder_storage == ContextFolderStorage.active_context_folder_storage):
            ...
            if kind=='common':
                obj = context_folder_storage.makeExpression(obj_id)
            elif kind=='axiom':
                obj = context_folder_storage.makeJudgmentOrProof(obj_id)
...
class ContextFolderStorage:
    ...
    def unload(self):
        ...
        proveit_obj_to_storage = ContextFolderStorage.proveit_object_to_storage
        to_remove = set()
        for obj_id, (context_folder_storage, hash_id) \
        ...
        if folder=='theorems':
            self.context_storage._theorem_names = None
            self.context_storage._loadedTheorems = dict()
        if folder=='common':
            self.context_storage._common_exp_names = None
            self.context_storage._loadedCommonExprs = dict()
        self.context_storage._special_hash_ids[kind] = None
    ...
    def _record_storage(self, obj_style_id, hash_id):
        ...
        if (ContextFolderStorage.owns_active_storage and
                self == ContextFolderStorage.active_context_folder_storage):
            ContextFolderStorage.owned_hash_folders.add(hash_id)
    ...
    def _owningNotebook(self):
        ...
        folder.
        '''
        directory = self.context._storage.directory
    ...
    def _generateObjectNotebook(self, proveItObject):
        ...
        if (ContextFolderStorage.owns_active_storage and
                self == ContextFolderStorage.active_context_folder_storage):
            if isinstance(proveItObject, Expression):
                ContextFolderStorage.expressionNotebook(proveItObject)
            elif isinstance(proveItObject, Proof):
    ...
    def expressionNotebook(expr, nameKindContext=None,
        ...
        from proveit._core_.proof import Axiom, Theorem
        from .context import UnsetCommonExpressions
        ...
        obj_context_folder_storage, hash_directory = \
            context_folder_storage._retrieve(obj)
        assert obj_context_folder_storage==context_folder_storage
        ...
        if nb is None:
            ...
            if not os.path.isfile(filepath):
                # disable for now
                #context_folder_storage._raiseNotebookNotFound(filepath)
                pass
            return relurl(filepath)
        
        #assert False,  '%s'%ContextFolderStorage.active_context_folder_storage
        
        # Possibly toggle expr.ipynb and alt_expr.ipynb.
        clean_nb = ContextFolderStorage._get_clean_nb_method()
        nb = nb.strip()
    ...
    def _get_clean_nb_method():
        ...
        return ContextFolderStorage.clean_nb_method
    ...
    def _makeExpression(self, expr_id, importFn, exprBuilderFn):
        ...
        for expr_id in reversed(expr_ids):
            ...
            if expr_style_id not in proveit_obj_to_storage:
                ...
                context_folder_storage, hash_id = \
                    exprid_to_storage[expr_id]
                context_folder_storage._record_storage(
    ...
    def makeJudgmentOrProof(self, storage_id):
        ...
        if unique_rep[:6] == 'Proof:':
            ...
            assert context_name == self.context.name
            assert len(subids)==1, (
            ...
            if kind=='axiom':
                ...
                obj = Axiom(judgment.expr, context, name)
            elif kind=='theorem':
                judgment = self.makeJudgmentOrProof(judgment_id)
                obj = Theorem(judgment.expr, context, name)
        elif unique_rep[:9] == 'Judgment:':
        ...
        context_folder_storage._record_storage(obj._style_id,
                                               hash_folder)
        return obj
    ...
    def clean(self, clear=False):
        ...
        owned_hash_folders = ContextFolderStorage.owned_hash_folders
