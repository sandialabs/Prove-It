...
class _ExprProofs:
    ...
    def best_proof(self, judgment):
        ...
        for proof in self._proofs:
            if proof.proven_truth.assumptions_set.issubset(judgment.assumptions_set):
...
class Judgment:
    ...
    def __init__(self, expression, assumptions):
        ...
        self._style_data = style_data(self._generate_unique_rep(lambda expr : hex(expr._style_id)))
    ...
    def _generate_unique_rep(self, object_rep_fn):
        ...
        '''
        return object_rep_fn(self.expr) + ';[' + ','.join(object_rep_fn(assumption) for assumption in self.assumptions) + ']'
            ...
            try:
                for side_effect in self.expr.side_effects(self):
                    ...
                    try:
                        ...
                        raise Exception("Side effect failure for %s, while running %s: "%(str(self.expr), str(side_effect)) + str(e))
            finally:
    ...
    def begin_proof(self, theorem):
        ...
        if Judgment.theorem_being_proven is not None:
            raise ProofInitiationFailure("May only begin_proof once per Python/IPython session.  Restart the notebook to restart the proof.")
        if not isinstance(theorem, Theorem):
            raise TypeError('Only begin a proof for a Theorem')
        if theorem.proven_truth != self:
            raise ValueError('Inconsistent theorem for the Judgment in begin_proof call')
        ...
        if str(self) in presumptions:
            ...
            # extra sanity check (should be caught within get_all_presumed_theorem_names)
        ...
        theorem._meaning_data._unusable_proof = theorem # can't use itself to prove itself
        return self.expr
    ...
    def print_requirements(self):
        ...
        if len(required_theorems) > 0:
            assert not is_fully_proven(self), "certification database is corrupt"
            print("\n_unproven theorems:")
    ...
    def _addProof(self, newproof=None):
        ...
        for expr_judgment in expr_judgments:
            ...
            if newproof.proven_truth.assumptions_set.issubset(expr_judgment.assumptions_set):
                # replace if there was no pre-existing usable proof or the new proof has fewer steps
                preexisting_proof = expr_judgment.proof()
                if preexisting_proof is None or not preexisting_proof.is_usable() or newproof.num_steps()<preexisting_proof.num_steps():
    ...
    def _updateProof(self, new_proof):
        ...
        if new_proof is None:
            ...
            if meaning_data._proof is not None:
                assert not meaning_data._proof.is_usable(), "should not update to an unusable new proof if the old one was usable"
    ...
    def with_matching_styles(self, expr, assumptions):
        ...
        for assumption in self.assumptions:
            if assumption in assumptions_dict:
                assumption.with_matching_style(assumptions_dict.pop(assumption))
        proof = self.proof()
    ...
    def find_judgment(expression, assumptions_set):
        ...
        if len(suitable_truths)==0: return None # no suitable truth
    ...
    def raise_unusable_proof(self):
        ...
        if proof == unusuable_proof:
            ...
            raise UnusableProof(Judgment.theorem_being_proven, unusuable_proof, 'required to prove' + self.string(perform_usability_check=False)) 
    ...
    def string(self, perform_usability_check=True):
        ...
        if perform_usability_check and not self.is_usable(): self.raise_unusable_proof()
        if len(self.assumptions) > 0:
            assumptions_str = ExprTuple(*self.assumptions).formatted('string', fence=False)
    ...
    def latex(self, perform_usability_check=True):
        ...
        if perform_usability_check and not self.is_usable(): self.raise_unusable_proof()
        if len(self.assumptions) > 0:
            assumptions_latex = ExprTuple(*self.assumptions).formatted('latex', fence=False)
    ...
    def __repr__(self):
        ...
        Return a string representation of the Judgment.
        '''
        if not self.is_usable(): self.raise_unusable_proof()
        return self.string()
...
def as_expressions(*truth_or_expressions):
    ...
    '''
    return [as_expression(truth_or_expression) for truth_or_expression in truth_or_expression]
