...
def _defineAxioms():
    ...
    concatenation_in_kleene = Forall([multi_a, X], Implies(In(List(multi_a), KleeneRepetition(X)), In(List(multi_a, X), KleeneRepetition(X))))
    return _firstAxiom, locals()

def _defineTheorems():
    # Forall_{A**, B**} ([A**] in X*) and ([B**] in X*) => [A**, B**] in X**
    _firstAxiom =\
    combining_in_kleene = Forall([multi_a, X], Implies(In(List(multi_a), KleeneRepetition(X)), In(List(multi_a, X), KleeneRepetition(X))))
    return None
