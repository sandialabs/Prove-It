...
deriveStmtEqTrue(falseEqFalse).lhsSubstitute(Or(FALSE, X), X).proven()
# [FALSE=TRUE or FALSE=FALSE] via [FALSE or FALSE=FALSE] and Not(FALSE=TRUE)
falseNotTrue.unfold().equateNegatedToFalse().lhsSubstitute(Or(X, Equals(FALSE, FALSE)), X).proven()
