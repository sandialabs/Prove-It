...
class Card(Function):
    ...
    def distinct_subset_existence(self, elems=None, assumptions=USE_DEFAULTS):
        ...
        if len(elems)==2:
            a_var, b_var = elems
            return distinct_pair_existence.instantiate({S:self.domain, a:a_var, b:b_var}, assumptions=assumptions)
        else:
            return distinct_subset_existence.instantiate({S:self.domain, N:num(len(elems)), x_multi:elems}, assumptions=assumptions)
