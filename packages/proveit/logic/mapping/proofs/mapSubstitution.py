...
fmap_x_eq_fx = mapping.mapApplication.instantiate().instantiate({x:y, y:x}).prove({Qx})
# [(y | Q(y)) -> g(y)](x) = g(x), assuming Q(x)
gmap_x_eq_gx = mapping.mapApplication.instantiate({f:g}).instantiate({x:y, y:x}).prove({Qx})
# [(y | Q(y)) -> f(y)](x) = g(x), assuming hypothesis, Q(x)
fmap_x_eq_gx = fmap_x_eq_fx.applyTransitivity(hypothesis.instantiate()).prove({hypothesis, Qx})
...
mapping.lambdaDomainDef.instantiate().instantiate({x:y, y:x}).deriveRightImplication().prove(Qx)
...
domain_equality = mapping.lambdaDomainEquality.instantiate({x:y}).prove()
...
mapping.qed('mapSubstitution', Implies(hypothesis, mapping.mapIsAsMapDoes.instantiate({f:fyGivenQyMap, g:gyGivenQyMap}).deriveConclusion()).generalize([f, g, Q]))
