...
class Or(Operation):
    ...
    _operator_ = Literal(string_format='or', latex_format=r'\lor', theory=__file__)

    trivial_disjunctions = set() #used to avoid infinite recursion inside of unary_reduction
    ...
    def conclude(self, assumptions=USE_DEFAULTS):
        ...
        for use_automation_for_operand in [False, True]:
            ...
            for _k, operand in enumerate(self.operands):
                try:
                    operand.prove(assumptions, automation=use_automation_for_operand)
                    proven_operand_indices.append(_k)
                    self.conclude_via_example(operand, assumptions=assumptions) # possible way to prove it
                except ProofFailure:
                    pass
            ...
            if len(proven_operand_indices) > 0:
                try:
                    ...
                    # or_if[Any,Left,Right] must not have been a usable theorem; use the default below.
                    break
    ...
    def conclude_negation(self, assumptions):
        ...
        if self == false_or_false_negated.operand:
            ...
            return neither_intro.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
        else:
            from proveit.numbers import num
            return not_or_if_not_any.instantiate({m: num(len(self.operands)), A: self.operands}, assumptions=assumptions)

    def conclude_via_both(self, assumptions):
        from ._theorems_ import or_if_both
        assert len(self.operands) == 2
        return or_if_both.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)

    def conclude_via_only_left(self, assumptions):
        from ._theorems_ import or_if_only_left
        assert len(self.operands) == 2
        return or_if_only_left.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
    ...
    def conclude_via_left(self, assumptions):
        ...
        assert len(self.operands) == 2
        return or_if_left.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)

    def conclude_via_only_right(self, assumptions):
        from ._theorems_ import or_if_only_right
        assert len(self.operands) == 2
        return or_if_only_right.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
    ...
    def conclude_via_demorgans(self, assumptions=USE_DEFAULTS):
        ...
        from proveit.numbers import num
        if len(self.operands) == 2:
            return demorgans_law_and_to_or_bin.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
        else:
            return demorgans_law_and_to_or.instantiate({m:num(len(self.operands)), A:self.operands}, assumptions=assumptions)
    ...
    def derive_right_if_not_left(self, assumptions=USE_DEFAULTS):
        ...
        return right_if_not_left.instantiate({A:left_operand, B:right_operand}, assumptions=assumptions)#.derive_conclusion(assumptions)
    ...
    def derive_left_if_not_right(self, assumptions=USE_DEFAULTS):
        ...
        assert len(self.operands) == 2
        left_operand, right_operand = self.operands
        return left_if_not_right.instantiate({A:left_operand, B:right_operand}, assumptions=assumptions)#.derive_conclusion(assumptions)

    def derive_via_singular_dilemma(self, conclusion, assumptions=USE_DEFAULTS):
        '''
        ...
        if len(self.operands) == 2:
            return singular_constructive_dilemma.instantiate({A:self.operands[0], B:self.operands[1], C:conclusion}, assumptions=assumptions)
        from proveit.numbers import num
        return singular_constructive_multi_dilemma.instantiate({m: num(len(self.operands)), A: self.operands, C:conclusion}, assumptions=assumptions)
    ...
    def derive_via_multi_dilemma(self, conclusion, assumptions=USE_DEFAULTS):
        ...
        from proveit.numbers import num
        assert isinstance(conclusion, Or) and len(conclusion.operands) == len(self.operands), "derive_via_multi_dilemma requires conclusion to be a disjunction, the same number of operands as self."
        ...
        if all(isinstance(operand, Not) for operand in self.operands) and all(isinstance(operand, Not) for operand in conclusion.operands):
            ...
            if len(self.operands) == 2 and destructive_dilemma.is_usable():
                ...
                return destructive_dilemma.instantiate({C:self.operands[0].operand, D:self.operands[1].operand, A:conclusion.operands[0].operand, B:conclusion.operands[1].operand}, assumptions=assumptions)
                ...
                negated_operands_self = [operand.operand for operand in self.operands]
                negated_operands_conc = [operand.operand for operand in conclusion.operands]
                return destructive_multi_dilemma.instantiate({m: num(len(self.operands)), A: negated_operands_self, B: negated_operands_conc}, assumptions=assumptions)
        # constructive case.
        if len(self.operands) == 2:
            # From (A or B), A => C, B => D, conclude C or D.
            return constructive_dilemma.instantiate({A:self.operands[0], B:self.operands[1], C:conclusion.operands[0], D:conclusion.operands[1]}, assumptions=assumptions)
        #raise NotImplementedError("Generalized constructive multi-dilemma not implemented yet.")
        return constructive_multi_dilemma.instantiate({m: num(len(self.operands)), A: self.operands, B: conclusion.operands},assumptions=assumptions)
    ...
    def deduce_left_in_bool(self, assumptions=USE_DEFAULTS):
        ...
        if len(self.operands) == 2:
            return left_in_bool.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
    ...
    def deduce_right_in_bool(self, assumptions=USE_DEFAULTS):
        ...
        if len(self.operands) == 2:
            return right_in_bool.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
    ...
    def deduce_part_in_bool(self, index_or_expr, assumptions=USE_DEFAULTS):
        ...
        from proveit.numbers import num
        idx = index_or_expr if isinstance(index_or_expr, int) else list(self.operands).index(index_or_expr)
        if idx < 0 or idx >= len(self.operands):
            raise IndexError("Operand out of range: " + str(idx))
        if len(self.operands)==2:
            if idx==0: return self.deduce_left_in_bool(assumptions)
            elif idx==1: return self.deduce_right_in_bool(assumptions)
        #attempt to replace with AA and CC over Amulti and Cmulti
        return each_is_bool.instantiate({m:num(idx), n:num(len(self.operands)-idx-1), A:self.operands[:idx], B:self.operands[idx], C:self.operands[idx+1:]}, assumptions=assumptions)
    ...
    def deduce_not_left_if_neither(self, assumptions=USE_DEFAULTS):
        ...
        assert len(self.operands) == 2
        left_operand, right_operand = self.operands
        return not_left_if_neither.instantiate({A:left_operand, B:right_operand}, assumptions=assumptions)
    ...
    def deduce_not_right_if_neither(self, assumptions=USE_DEFAULTS):
        ...
        assert len(self.operands) == 2
        left_operand, right_operand = self.operands
        return not_right_if_neither.instantiate({A:left_operand, B:right_operand}, assumptions=assumptions)
    ...
    def derive_common_conclusion(self, conclusion, assumptions=USE_DEFAULTS):
        ...
        return hypothetical_disjunction.instantiate({A:left_operand, B:right_operand, C:conclusion}, assumptions=assumptions).derive_conclusion(assumptions).derive_conclusion(assumptions)
    ...
    def evaluation(self, assumptions=USE_DEFAULTS, *, automation=True,
        ...
        from ._axioms_ import or_t_t, or_t_f, or_f_t, or_f_f # load in truth-table evaluations
        if len(self.operands)==0:
            return self.unary_reduction(assumptions=assumptions)
    ...
    def derive_contradiction(self, assumptions=USE_DEFAULTS):
        ...
        if len(self.operands) == 2:
            return binary_or_contradiction.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
        else:
            from proveit.numbers import num
            return or_contradiction.instantiate({m:num(len(self.operands)), A:self.operands}, assumptions=assumptions)
    ...
    def deduce_in_bool(self, assumptions=USE_DEFAULTS):
        ...
        from proveit.numbers import num
        if len(self.operands) == 2:
            return binary_closure.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
        else:
    ...
    def conclude_via_example(self, true_operand, assumptions=USE_DEFAULTS):
        ...
        if len(self.operands) == 2:
            if index == 0:
                ...
                return or_if_right.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
        return or_if_any.instantiate({m:num(index), n:num(len(self.operands)-index-1), A:self.operands[:index], B:self.operands[index], C:self.operands[index+1:]}, assumptions=assumptions)
    ...
    def group_commutation(self, init_idx, final_idx, length, disassociate=True,
        ...
        '''
        return group_commutation(self, init_idx, final_idx, length, disassociate,
                                assumptions)
    ...
    def commute(self, init_idx=None, final_idx=None, assumptions=USE_DEFAULTS):
        ...
        return apply_commutation_thm(self, init_idx, final_idx, commute, leftward_commute, rightward_commute, assumptions)

    def group_commute(self, init_idx, final_idx, length, disassociate=True, assumptions=USE_DEFAULTS):
        '''
        ...
        '''
        return group_commute(self, init_idx, final_idx, length, disassociate, assumptions)
    ...
    def association(self, start_idx, length, assumptions=USE_DEFAULTS):
        ...
        return apply_association_thm(self, start_idx, length, association, assumptions)
    ...
    def associate(self, start_idx, length, assumptions=USE_DEFAULTS):
        ...
        return apply_association_thm(self, start_idx, length, associate, assumptions)
...
InnerExpr.register_equivalence_method(Or, 'group_commutation', 'group_commuted', 'group_commute')
