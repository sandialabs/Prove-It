...
class BooleanSet(Literal):
    def __init__(self):
        Literal.__init__(self, string_format='BOOLEAN', latex_format=r'\mathbb{B}')
    ...
    def forall_evaluation(self, forall_stmt, assumptions=USE_DEFAULTS):
        ...
        assert(isinstance(forall_stmt, Forall)), "May only apply forall_evaluation method of BOOLEAN to a forall statement"
        assert(forall_stmt.domain == Boolean), "May only apply forall_evaluation method of BOOLEAN to a forall statement with the BOOLEAN domain"
        ...
        if true_instance == TRUE and false_instance == FALSE:
            ...
            return forall_bool_eval_false_via_t_f.instantiate({P_op:instance_expr}).derive_conclusion()
        else:
            ...
            if not isinstance(eval_true_instance.expr, Equals) or not isinstance(eval_false_instance.expr, Equals):
            ...
            if true_case_val == TRUE and false_case_val == TRUE:
                ...
                compose([eval_true_instance.derive_via_boolean_equality(), eval_false_instance.derive_via_boolean_equality()], assumptions)
                forall_bool_eval_true.instantiate({P_op:instance_expr, A:instance_var})
                return forall_bool_eval_true.instantiate({P_op:instance_expr, A:instance_var}, assumptions=assumptions).derive_conclusion(assumptions)
            else:
                ...
                if true_case_val == FALSE and false_case_val == FALSE:
                    return forall_bool_eval_false_via_f_f.instantiate({P_op:instance_expr, A:instance_var}, assumptions=assumptions).derive_conclusion(assumptions)
                elif true_case_val == FALSE and false_case_val == TRUE:
                    return forall_bool_eval_false_via_f_t.instantiate({P_op:instance_expr, A:instance_var}, assumptions=assumptions).derive_conclusion(assumptions)
                elif true_case_val == TRUE and false_case_val == FALSE:
                    return forall_bool_eval_false_via_t_f.instantiate({P_op:instance_expr, A:instance_var}, assumptions=assumptions).derive_conclusion(assumptions)
                else:
    ...
    def unfold_forall(self, forall_stmt, assumptions=USE_DEFAULTS):
        ...
        assert(isinstance(forall_stmt, Forall)), "May only apply unfold_forall method of Boolean to a forall statement"
        assert(forall_stmt.domain == Boolean), "May only apply unfold_forall method of Boolean to a forall statement with the Boolean domain"
    ...
    def fold_as_forall(self, forall_stmt, assumptions=USE_DEFAULTS):
        ...
        from ._common_ import Boolean
        assert(isinstance(forall_stmt, Forall)), "May only apply fold_as_forall method of Boolean to a forall statement"
        assert(forall_stmt.domain == Boolean), "May only apply fold_as_forall method of Boolean to a forall statement with the Boolean domain"
...
class BooleanMembership(Membership):
    ...
    def side_effects(self, judgment):
        ...
        if hasattr(self.element, 'in_bool_side_effects'):
            for side_effect in self.element.in_bool_side_effects(judgment):
                yield side_effect
        # don't automatically do unfold_is_bool_explicit if unfold_is_bool is unusable -- avoids infinite recursion
    ...
    def conclude(self, assumptions=USE_DEFAULTS):
        ...
        # if the element is already proven or disproven, use in_bool_if_true or in_bool_if_false
        try:
            element.prove(assumptions=assumptions, automation=False)
            return in_bool_if_true.instantiate({A:element}, assumptions=assumptions)
        except ProofFailure:
            pass
        try:
            element.disprove(assumptions=assumptions, automation=False)
            return in_bool_if_false.instantiate({A:element}, assumptions=assumptions)
        except ProofFailure:
            pass
        ...
        raise ProofFailure(in_bool(element), assumptions, str(element) + ' not proven to be equal to TRUE or FALSE.')
    ...
    def unfold(self, assumptions=USE_DEFAULTS):
        ...
        if unfold_is_bool.is_usable():
            ...
            return unfold_is_bool.instantiate({A:self.element}, assumptions=assumptions)
        else:
            #  [(element = TRUE) or (element = FALSE)] assuming in_bool(element)
            return unfold_is_bool_explicit.instantiate({A:self.element}, assumptions=assumptions)
    ...
    def fold(self, assumptions=USE_DEFAULTS):
        ...
        if fold_is_bool.is_usable():
            return fold_is_bool.instantiate({A:self.element}, assumptions=assumptions)
    ...
    def derive_via_excluded_middle(self, consequent, assumptions=USE_DEFAULTS):
        ...
        return from_excluded_middle.instantiate({A:self.element, C:consequent}, assumptions=assumptions)
