...
singleton_def.instantiate({x:A, y:TRUE}).sub_right_side_into(Or(X, In(A, Singleton(FALSE))), X).proven({in_bool(A)})
# A=TRUE or A=FALSE assuming in_bool(A)
conclusion = singleton_def.instantiate({x:A, y:FALSE}).sub_right_side_into(Or(Equals(A, TRUE), X), X).proven({in_bool(A)})
