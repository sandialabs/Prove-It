...
def forall_bool_eval_false_derivation(PofTrueVal, PofFalseVal):
    ...
    hypothesis.derive_left().in_bool_via_boolean_equality().proven({hypothesis})
    # P(FALSE) in BOOLEANS assuming hypothesis
    hypothesis.derive_right().in_bool_via_boolean_equality().proven({hypothesis})
    # forall_{A in BOOLEANS} P(A) in BOOLEANS assuming hypothesis
    Forall(A, in_bool(PofA), domain=BOOLEANS).conclude_as_folded().proven({hypothesis})
    if PofTrueVal == FALSE:
        # Not(P(TRUE)) assuming hypothesis
        hypothesis.derive_left().derive_via_boolean_equality().proven({hypothesis})
        example = TRUE
        # TRUE in BOOLEANS
        true_is_bool
    elif PofFalseVal == FALSE:
        # Not(P(FALSE)) assuming hypothesis
        hypothesis.derive_right().derive_via_boolean_equality().proven({hypothesis})
        example = FALSE    
        # FALSE in BOOLEANS
        false_is_bool
    # [forall_{A in BOOLEANS} P(A)] = FALSE assuming hypothesis
    conclusion = Exists(A, Not(PofA), domain=BOOLEANS).conclude_via_example(example).derive_negated_forall().equate_negated_to_false().proven({hypothesis})
