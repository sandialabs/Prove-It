...
class Exists(OperationOverInstances):
    ...
    _operator_ = Literal(string_format='exists', latex_format=r'\exists', theory=__file__)
    ...
    def definition(self, assumptions=USE_DEFAULTS):
        ...
        _P = Lambda(self.instance_params, self.operand.body.value)
        _Q = Lambda(self.instance_params, self.operand.body.condition)
        return exists_def.instantiate({n:_n, P:_P, Q:_Q}, assumptions=assumptions)
    ...
    def deduce_not_exists(self, assumptions=USE_DEFAULTS):
        ...
        not_exists_expr = NotExists(self.instance_vars, self.instance_expr, domain=self.domain, conditions=self.conditions)
    ...
    def conclude_via_example(self, example_instance, assumptions=USE_DEFAULTS):
        ...
        if len(self.instance_vars) > 1 and (not isinstance(example_instance, ExprTuple) or (len(example_instance) != len(self.instance_vars))):
            raise Exception('Number in example_instance list must match number of instance variables in the Exists expression')
        ...
        example_mapping = {instance_var:example_instance_elem for instance_var, example_instance_elem in zip(self.instance_vars, example_instance if isinstance(example_instance, ExpressionList) else [example_instance])}
        ...
        # exists_{..y.. | ..Q(..x..)..} P(..y..)]
        return existence_by_example.instantiate({P_op:P_op_sub, Q_op:Q_op_sub, S:self.domain}, assumptions=assumptions,
                                              relabel_map={x_multi:example_instance, y_multi:self.instance_vars}).derive_consequent(assumptions=assumptions)
    ...
    def derive_negated_forall(self, assumptions=USE_DEFAULTS):
        ...
        if isinstance(self.instance_expr, Not):
            P_op, P_op_sub = Operation(P, self.instance_vars), self.instance_expr.operand
            return exists_not_implies_not_forall.instantiate({P_op:P_op_sub, Q_op:Q_op_sub, S:self.domain}, assumptions=assumptions,
                                                        relabel_map={x_multi:self.instance_vars}).derive_consequent(assumptions)
        else:
            P_op, P_op_sub = Operation(P, self.instance_vars), self.instance_expr
            return exists_def.instantiate({P_op:P_op_sub, Q_op:Q_op_sub, S:self.domain}, assumptions=assumptions,
                                        relabel_map={x_multi:self.instance_vars}).derive_right_via_equality(assumptions)
    ...
    def substitute_domain(self, superset, assumptions=USE_DEFAULTS):
        ...
        P_op, P_op_sub = Operation(P, self.instance_vars), self.instance_expr
        Q_op, Q_op_sub = Operation(Qmulti, self.instance_vars), self.conditions
        return exists_in_superset.instantiate({P_op:P_op_sub, Q_op:Q_op_sub, A:self.domain, B:superset}, assumptions=assumptions,
                                            relabel_map={x_multi:self.instance_vars, y_multi:self.instance_vars}).derive_consequent(assumptions)
    ...
    def elim_domain(self, assumptions=USE_DEFAULTS):
        ...
        P_op, P_op_sub = Operation(P, self.instance_vars), self.instance_expr
        Q_op, Q_op_sub = Operation(Qmulti, self.instance_vars), self.conditions
        return exists_in_general.instantiate({P_op:P_op_sub, Q_op:Q_op_sub, S:self.domain}, assumptions=assumptions,
                                           relabel_map={x_multi:self.instance_vars, y_multi:self.instance_vars}).derive_consequent(assumptions)
    ...
    def deduce_in_bool(self, assumptions=USE_DEFAULTS):
        ...
        P_op, P_op_sub = Operation(P, self.instance_vars), self.instance_expr
        Q_op, Q_op_sub = Operation(Qmulti, self.instance_vars), self.conditions
        return exists_is_bool.instantiate({P_op:P_op_sub, Q_op:Q_op_sub, S:self.domain}, relabel_map={x_multi:self.instance_vars}, assumptions=assumptions)
    ...
    def substitute_instances(self, universality, assumptions=USE_DEFAULTS):
        ...
        if self.instance_expr in universality.conditions:
            ...
            first_condition = universality.conditions[0].substituted(i_var_substitutions)
            ...
            if ExpressionList(universality.conditions[1:]).substituted(i_var_substitutions) != self.conditions:
            ...
            P_op, P_op_sub = Operation(P, self.instance_vars), self.instance_expr
            Q_op, Q_op_sub = Operation(Qmulti, self.instance_vars), self.conditions
            R_op, R_op_sub = Operation(R, self.instance_vars), universality.instance_expr.substituted(i_var_substitutions)
            if self.has_domain():
                return existential_implication.instantiate({S:self.domain, P_op:P_op_sub, Q_op:Q_op_sub, R_op:R_op_sub}, \
                                                        relabel_map={x_multi:universality.instance_vars, y_multi:self.instance_vars, z_multi:self.instance_vars}, assumptions=assumptions).derive_consequent(assumptions).derive_consequent(assumptions)
            else:
                return no_domain_existential_implication.instantiate({P_op:P_op_sub, Q_op:Q_op_sub, R_op:R_op_sub}, 
                                                                   relabel_map={x_multi:universality.instance_vars, y_multi:self.instance_vars, z_multi:self.instance_vars}, assumptions=assumptions).derive_consequent(assumptions).derive_consequent(assumptions)
