...
class NotExists(OperationOverInstances):
    ...
    _operator_ = Literal(string_format='notexists', latex_format=r'\nexists', theory=__file__)
    ...
    def unfold(self, assumptions=USE_DEFAULTS):
        ...
        '''
        from ._theorems_ import not_exists_unfolding
        P_op, P_op_sub = Operation(P, self.instance_vars), self.instance_expr
        Q_op, Q_op_sub = Etcetera(Operation(Q, self.instance_vars)), self.conditions
        return not_exists_unfolding.instantiate({P_op:P_op_sub, Q_op:Q_op_sub, x_multi:self.instance_vars, S:self.domain}).derive_conclusion(assumptions)
    ...
    def conclude_as_folded(self, assumptions=USE_DEFAULTS):
        ...
        '''
        from ._theorems_ import not_exists_folding
        P_op, P_op_sub = Operation(P, self.instance_vars), self.instance_expr
        Q_op, Q_op_sub = Etcetera(Operation(Q, self.instance_vars)), self.conditions
        folding = not_exists_folding.instantiate({P_op:P_op_sub, Q_op:Q_op_sub, x_multi:self.instance_vars, S:self.domain})
    ...
    def conclude_via_forall(self):
        ...
        '''
        from ._theorems_ import forall_implies_not_exists_not, exists_def_negation
