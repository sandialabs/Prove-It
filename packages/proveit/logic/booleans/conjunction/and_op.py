...
class And(Operation):
    ...
    _operator_ = Literal(string_format='and', latex_format=r'\land', theory=__file__)
    ...
    def conclude(self, assumptions):
        ...
        if self == true_and_true.expr: return true_and_true # simple special case
    ...
    def conclude_negation(self, assumptions=USE_DEFAULTS):
        ...
        from ._theorems_ import (true_and_false_negated, false_and_true_negated, 
                                 false_and_false_negated, nand_if_neither, 
                                 nand_if_left_but_not_right, nand_if_right_but_not_left)
        from proveit.logic import Not
        not_self = Not(self)
        if not_self in {true_and_false_negated.expr, false_and_true_negated.expr, false_and_false_negated.expr}:
        ...
        for use_automation_for_operand in [False, True]:
            ...
            for _k, operand in enumerate(self.operands):
                try:
                    operand.disprove(assumptions, automation=use_automation_for_operand)
            ...
            if len(self.operands) == 2 and len(disproven_operand_indices) > 0:
                ...
                try:
                    if len(disproven_operand_indices) == 2:
                        return nand_if_neither.instantiate({A: self.operands[0], B: self.operands[1]}, assumptions=assumptions)
                    elif disproven_operand_indices[0] == 0:
                        return nand_if_right_but_not_left.instantiate({A: self.operands[0], B: self.operands[1]}, assumptions=assumptions)
                    else:
                        return nand_if_left_but_not_right.instantiate({A: self.operands[0], B: self.operands[1]}, assumptions=assumptions)
                except:
                    pass
            if len(disproven_operand_indices) > 0:
                ...
                try:
                    ...
                    return self.conclude_via_example(operand, assumptions=assumptions)
    ...
    def derive_left(self, assumptions=USE_DEFAULTS):
        ...
        '''
        if len(self.operands) != 2:
            raise Exception('derive_left only applicable for binary conjunction operations')
    ...
    def derive_right(self, assumptions=USE_DEFAULTS):
        ...
        '''
        if len(self.operands) != 2:
            raise Exception('derive_right only applicable for binary conjunction operations')
    ...
    def deduce_left_in_bool(self, assumptions=USE_DEFAULTS):
        ...
        if len(self.operands) == 2:
            left_in_bool.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
    ...
    def deduce_right_in_bool(self, assumptions=USE_DEFAULTS):
        ...
        if len(self.operands) == 2:
            right_in_bool.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
    ...
    def deduce_part_in_bool(self, index_or_expr, assumptions=USE_DEFAULTS):
        ...
        if len(self.operands)==2:
            ...
            return each_is_bool.instantiate({m:m_val, n:n_val, A:self.operands[:idx], B:self.operands[idx], C:self.operands[idx+1:]}, assumptions=assumptions)
    ...
    def conclude_via_demorgans(self, assumptions=USE_DEFAULTS):
        ...
        from proveit.numbers import num
        if len(self.operands) == 2:
            return demorgans_law_or_to_and_bin.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
        else:
            return demorgans_law_or_to_and.instantiate({m:num(len(self.operands)), A:self.operands}, assumptions=assumptions)
    ...
    def conclude_via_example(self, true_operand, assumptions=USE_DEFAULTS):
        ...
        if len(self.operands) == 2:
            if index == 0:
                return nand_if_not_left.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
            elif index == 1:
                return nand_if_not_right.instantiate({A:self.operands[0], B:self.operands[1]}, assumptions=assumptions)
        return nand_if_not_one.instantiate({m:num(index), n:num(len(self.operands)-index-1), A:self.operands[:index], B:self.operands[index], C:self.operands[index+1:]}, assumptions=assumptions)
    ...
    def conclude_as_redundant(self, assumptions=USE_DEFAULTS):
        ...
        if self.operands[0].start_index != one:
            raise NotImplementedError("'conclude_as_redundant' only implemented "
    ...
    def evaluation(self, assumptions=USE_DEFAULTS, *, automation=True,
        ...
        from ._axioms_ import and_t_t, and_t_f, and_f_t, and_f_f # load in truth-table evaluations
        if len(self.operands)==0:
            return self.unary_reduction(assumptions)
    ...
    def commutation(self, init_idx=None, final_idx=None, assumptions=USE_DEFAULTS):
        ...
        return apply_commutation_thm(self, init_idx, final_idx, commutation, leftward_commutation, rightward_commutation, assumptions)
...
def compose(*expressions, assumptions=USE_DEFAULTS):
    ...
    if len(expressions)==2:
        ...
        return and_if_both.instantiate({A:expressions[0], B:expressions[1]}, assumptions=assumptions)
    else:
        from proveit.numbers import num
        from ._theorems_ import and_if_all
        return and_if_all.instantiate({m:num(len(expressions)), A:expressions}, assumptions=assumptions)

# Register these expression equivalence methods:
InnerExpr.register_equivalence_method(And, 'commutation', 'commuted', 'commute')
InnerExpr.register_equivalence_method(And, 'group_commutation', 'group_commuted', 'group_commute')
