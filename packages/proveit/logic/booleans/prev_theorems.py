...
by_cases_weak = Forall(
    (A, B), B, domain=Boolean, conditions=[
        Implies(
            A, B), Implies(
                Not(A), B)])
...
by_cases = Forall(
    A, Forall(
        B, B, conditions=[
            Implies(
                A, B), Implies(
                    Not(A), B)]), domain=Boolean)

fold_forall_over_bool = Forall(
    P,
    Forall(
        A,
        PofA,
