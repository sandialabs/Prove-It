...
# [TRUE or FALSE]
orTF.deriveViaBooleanEquality().prove()
# [TRUE or TRUE=FALSE] via [TRUE or FALSE] and TRUE != FALSE
trueNotFalse.unfold().equateNegatedToFalse().lhsSubstitute(X, Or(TRUE, X)).prove()
# [TRUE=TRUE or TRUE=FALSE] via [TRUE or TRUE=FALSE] and TRUE=TRUE
deriveStmtEqTrue(trueEqTrue).lhsSubstitute(X, Or(X, Equals(TRUE, FALSE))).prove()
