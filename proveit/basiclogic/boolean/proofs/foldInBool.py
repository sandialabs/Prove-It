...
singletonDef.specialize({x:A, y:FALSE}).lhsSubstitute(Or(Equals(A, TRUE), X), X).proven({hypothesis})
# (A in {TRUE}) or (A in {FALSE}) assuming hypothesis
singletonDef.specialize({x:A, y:TRUE}).lhsSubstitute(Or(X, In(A, Singleton(FALSE))), X).proven({hypothesis})
...
boolsDef.lhsSubstitute(In(A, X), X).proven({hypothesis})
