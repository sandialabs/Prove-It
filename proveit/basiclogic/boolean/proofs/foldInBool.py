...
singletonDef.specialize({x:A, y:FALSE}).lhsSubstitute(X, Or(Equals(A, TRUE), X)).proven({hypothesis})
# (A in {TRUE}) or (A in {FALSE}) assuming hypothesis
singletonDef.specialize({x:A, y:TRUE}).lhsSubstitute(X, Or(X, In(A, Singleton(FALSE)))).proven({hypothesis})
...
boolsDef.lhsSubstitute(X, In(A, X)).proven({hypothesis})
