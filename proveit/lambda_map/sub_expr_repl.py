...
class SubExprRepl:
    ...
    `SubExprMap(expr).operands[2].numerator` to obtain the SubExprMap
    ...
    def __init__(self, masterExpr, subExprPath=tuple()):
        '''
        Create an SubExprMap with the given master Expression.
        ...
        SubExprMap is generated appropriately, extending the 
    ...
    def __getitem__(self, key):
        '''
        If the currently mapped sub-Expression of the SubExprMap
        is a Composite, accessing an item corresponding to an item
        of this Composite will return the SubExprMap with the extended 
        path to this item.
        '''
        ...
        if isinstance(curSubExpr, ExpressionList):
            ...
            return SubExprMap(self.exprHierarchy[0], self.subExprPath + (key,))
            ...
            return SubExprMap(self.exprHierarchy[0], self.subExprPath + [sortedKeys.index(key)])
    ...
    def __getattr__(self, name):
        ...
        return the SubExprMap with the extended path to the sub-expression.
        '''
        ...
        if isinstance(curSubExpr.__dict__[name], Expression):
            ...
            # if so, return the SubExprMap extended to the sub-sub-Expression.
                ...
                if subSub == expr:
                    ...
                    if isinstance(expr, Composite):
                        ...
                        if withDummyVars.__dict__[name] == compositeWithDummyVars:
                            ...
                            return SubExprMap(self.exprHierarchy[0], self.subExprPath + (i,))                    
                    else:
                    ...
                    for j, subSubSub in enumerate(subSub):
                        if subSubSub == expr:
                            ...
                            if withDummyVar.__dict__[name] == dummyVar:
                                return SubExprMap(self.exprHierarchy[0], self.subExprPath + (i, j))
    ...
    def lambdaMap(self):
        '''
        Returns the lambda function/map that the SubExprMap is currently
        producing.
        '''
    ...
    def _expr_rep(self):
        ...
        '''
        lambdaFn = self.lambdaFn()
        lambdaArgs = lambdaFn.arguments
        curSubExpr = self.exprHierarchy[-1]
        if isinstance(curSubExpr, Composite):
            subExprs = list(curSubExpr.subExprIter())
        else:
            subExprs = [curSubExpr]
        namedExprDict = {'lambda':lambdaFn}
